...
| `make import-ops` | **Liner's own mail and demo bookings from another box, nothing generated.** `scripts/export_ops.py` runs on the old box with the standard library only (whatever commit it is on) and packs `ops_` bookings and mail, plus the unplaced `inbound_emails` addressed to us and their `.eml` files. It leaves every row on a reserved address (`.invalid`, `.example`), refused deliveries and 555 numbers. `FILE=... [ARGS=--apply]` reads it here: sent mail re-attached to its author by address, only `is_ours` mail kept, existing rows win |
