...
class EmailSender:
    ...
    def default_address(self, realm: str = "dealership") -> str:
        ...
        **Two realms, two mailboxes, and the split is not cosmetic.** A
        ...
        # mailbox on its mail domain (`sales@alsbou.linerai.us`) is that
        # store's, and the intake routes mail to it into that store. A profile
        # with none -- the fixture -- falls through to what the deployment
        # configured.
        from app import profile

        own = profile.mailbox_address()
        if own:
            return own
